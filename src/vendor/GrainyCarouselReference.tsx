...
function animate(from:number,to:number,opts:any){
  let stopped=false; const start=performance.now(); const dur=Math.max(1,(opts.duration??0.5)*1000);
  const ease=(t:number)=>1-Math.pow(1-t,3);
  let raf=0; const tick=(now:number)=>{if(stopped)return; const p=Math.min(1,(now-start)/dur); const v=from+(to-from)*ease(p); opts.onUpdate?.(v); if(p<1) raf=requestAnimationFrame(tick); else opts.onComplete?.();};
  raf=requestAnimationFrame(tick); return {stop(){stopped=true;cancelAnimationFrame(raf)}};
}
